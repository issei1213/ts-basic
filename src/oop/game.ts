...
class Positon {
  ...
  distanceForm(position: Positon, player: Player) {
    if(player === "first") {
      return {
...
abstract class Piece {
  ...
  constructor(private readonly player: Player, suji: Suji, dan: Dan) {
    ...
    this.position = new Positon(suji, dan);
    // result Piece { position: Positon { suji: 1, dan: '3' } }
  }

  // メソッドの定義
  // 駒の移動用のメソッド
  moveTo(position: Positon) {
    this.position = position;
  }

  // 移動できるかどうかを判定するメソッド
  abstract canMoveTo(position: Positon, player: Player): boolean;
}

class Osho extends Piece {
  // 王将のcanMoveToメソッドを具体的に実装する
  canMoveTo(position: Positon, player: Player): boolean {
